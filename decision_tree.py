...
class MultiNodeCategoricalDecisionTree(BaseEstimator, ClassifierMixin):
    ...
    def _build_tree(self, X: np.ndarray, y: np.ndarray, depth: int = 0) -> Dict[str, Any]:
        ...
        if ((depth>=self.max_depth and self.max_depth is not None )or (len(y)< self.min_samples_split)):
            ...
            node={'feature_idx': index , 'children':{},'majority_class': np.bincount(y).argmax()}
            ...
            for f in features:
                ...
                if len(new_y) > 0:
                    node["children"][f]=self._build_tree(new_x,new_y,depth+1)
                else:
                    label = np.argmax(np.bincount(y))
                    node["children"][f] = {"class": label}
    ...
    def _predict_single(self, x: np.ndarray) -> Any:
        ...
        while "class" not in node:
            ...
            # Traverse to the child node based on the feature value
            if feature_value in node["children"]:
                node = node["children"][feature_value]
            else:
                # Handle case where the feature value wasn't seen during training
                # You might want to return the majority class or some default class

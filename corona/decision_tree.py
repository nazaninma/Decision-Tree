...
class MultiNodeCategoricalDecisionTree(BaseEstimator, ClassifierMixin):
    ...
    def _build_tree(self, X: np.ndarray, y: np.ndarray, depth: int = 0) -> Dict[str, Any]:
        ...
        current=self._best_split_gini(X,y)
        idx=current['index']
        if(idx==-1):
            num=np.bincount(y)
            lable=np.argmax(num)
        ...
        node={'index' : idx,'children' : {},'set_class': np.bincount(y).argmax()}
        ...
        for v in feature_val:
            ...
            if len(new_y) > 0:
                node["children"][v]=self._build_tree_gini(new_x,new_y,depth+1)
            else:
        ...
        # current=self._best_split_entropy(X,y)
        # idx=current['index']
        # if(idx==-1):
        #     num=np.bincount(y)
        #     lable=np.argmax(num)
        ...
        #     new_x=np.delete(new_x,idx,axis=1)
        #     if len(new_y) > 0:
        #         node["children"][v]=self._build_tree_entropy(new_x,new_y,depth+1)
        #     else:
        #         label = np.argmax(np.bincount(y))
        #         node["children"][v] = {"class": label}
        # return node
    ...
    # def _build_tree_entropy(self, X: np.ndarray, y: np.ndarray, depth: int = 0) -> Dict[str, Any]:
        # """
        # Recursively build the decision tree.
    ...
    def _best_split_gini(self, X: np.ndarray, y: np.ndarray) -> Dict[str, Any]:
        """
        ...
        parent_entropy=self._calculate_gini(y)
        ...
        for i in range(num):
            ...
            information = parent_entropy- ent
            if(best_info<information):
    ...
    def _best_split_entropy(self, X: np.ndarray, y: np.ndarray) -> Dict[str, Any]:
                # also can use gini
        ...
        feature_dic={'index': feature_index}
        return feature_dic
    ...
    def _calculate_feature_entropy(self, X: np.ndarray, y: np.ndarray, index: int) -> float:
        """
    ...
    def _calculate_feature_gini(self, X: np.ndarray, y: np.ndarray, index: int) -> float:
        """
